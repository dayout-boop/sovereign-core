...
type OAuth2PKCEAdapter struct {
	...
	revoked         map[string]bool         // token(access/refresh) → revoked
	accessToRefresh map[string]string       // accessToken → refreshToken (연관 폐기용)
}
...
func (a *OAuth2PKCEAdapter) ExchangeCode(ctx context.Context, code, codeVerifier, redirectURI string) (*OAuth2Token, error) {
	...
	for _, sess := range a.sessions {
		...
		// AccessToken → RefreshToken 연관 추적 (AccessToken 폐기 시 RefreshToken도 무효화).
		a.accessToRefresh[token.AccessToken] = token.RefreshToken
		// 인가 코드 1회 사용 원칙 — 교환 즉시 세션 삭제.
...
func (a *OAuth2PKCEAdapter) RefreshToken(ctx context.Context, refreshToken string) (*OAuth2Token, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	// 폐기된 토큰 검사 — RevokeToken 호출 후 재사용 방지.
	if a.revoked[refreshToken] {
		return nil, fmt.Errorf("pkce: refresh token has been revoked")
	}
...
func (a *OAuth2PKCEAdapter) RevokeToken(ctx context.Context, token string) error {
	...
	// AccessToken 폐기 시 연관된 RefreshToken도 함께 무효화.
	if rt, ok := a.accessToRefresh[token]; ok {
		a.revoked[rt] = true
		delete(a.tokens, rt)
		delete(a.accessToRefresh, token)
	}
	// RefreshToken 직접 폐기 시.
...
func (a *DeviceFlowAdapter) RequestDeviceCode(ctx context.Context, clientID, scope string) (*DeviceCode, error) {
	// 빈 clientID 검증 — 미등록 클라이언트 차단.
	if clientID == "" {
		return nil, fmt.Errorf("device_flow: clientID is required")
	}
	dc := newID("dc")
...
// AuthorizeDeviceCode — 테스트/어드민용: 특정 deviceCode를 승인 처리.
...
type apiKeyRecord struct {
	...
	Revoked   bool   // 폐기 여부 — ListAPIKeys에서도 반환하여 감사 추적 가능
}
...
func (a *APIKeyAdapter) IssueAPIKey(ctx context.Context, orgID, name string, scopes []string, expiresAt time.Time) (string, *APIKeyInfo, error) {
	// 빈 orgID 검증.
	if orgID == "" {
		return "", nil, fmt.Errorf("api_key: orgID is required")
	}
	// 과거 만료 시간 검증.
	if !expiresAt.IsZero() && time.Now().After(expiresAt) {
		return "", nil, fmt.Errorf("api_key: expiresAt must be in the future, got %v", expiresAt)
	}
...
func (a *APIKeyAdapter) VerifyAPIKey(ctx context.Context, plainKey string) (*APIKeyInfo, error) {
	...
	rec := a.keys[keyID]
	if rec.Revoked {
		return nil, fmt.Errorf("api_key: key has been revoked")
	}
...
func (a *APIKeyAdapter) VerifyHMAC(ctx context.Context, keyID string, payload []byte, timestamp time.Time, signature string) error {
	...
	if rec.Revoked {
		return fmt.Errorf("api_key: key has been revoked")
	}
...
func (a *APIKeyAdapter) RevokeAPIKey(ctx context.Context, keyID string) error {
	...
	// 폐기 플래그 설정 — hashIdx에서는 제거하여 VerifyAPIKey 차단.
	// keys 맵에는 유지하여 ListAPIKeys 감사 추적 가능.
	rec.Revoked = true
...
func (a *APIKeyAdapter) ListAPIKeys(ctx context.Context, orgID string) ([]APIKeyInfo, error) {
	...
	for _, rec := range a.keys {
		if rec.Info.OrgID == orgID {
			// 폐기된 키도 포함 — 감사 추적을 위해 Status 필드에 반영.
			info := rec.Info
			if rec.Revoked {
				info.Status = "revoked"
			}
